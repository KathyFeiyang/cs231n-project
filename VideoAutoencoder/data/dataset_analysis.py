...
if __name__ == "__main__":
    ...
    for video_path in video_paths:
        ...
        importance_mask = full_reconstruct_im.mean()

        importance_mask.backward()

        true_mask = scene_rep.grad.abs()

        pil_im = to_image(full_reconstruct_im)
        print(scene_rep.shape)
        histo, _ = torch.max(true_mask, 1, keepdim=True)
        print(histo.shape)
        print(histo.mean(), histo.max(), histo.median())

        histo_rep = histo
        histo_scale = (histo_rep)/histo_rep.max() * 10
